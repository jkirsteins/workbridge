...
pub use set_branch::{PendingBranchAction, SetBranchDialog};
pub use slug::{MAX_SLUG_LEN, random_suffix, slugify, truncate_slug};
...
impl CreateDialog {
    ...
    pub fn open(&mut self, active_repos: &[PathBuf], cwd_repo: Option<&PathBuf>) {
        ...
        // latch (see rat-text-3.1.0 text_area: `clear` vs `set_text`). If
        // the user had scrolled the description on a previous open, that
        // stale offset would carry into the next open and hide the first
        // row of typed characters. Follow `clear()` with `set_text("")`
        // to force a full scroll-state reset - the same reset `set_text`
        // uses for normal assignment.
    ...
    /// quickstart submission path only consumes `selected[0]` and would
    /// silently discard any additional checked rows, so the dialog must
    /// never let the user check more than one repo at a time. In normal
    /// (Ctrl+B) mode this stays a plain toggle because the full create
    /// dialog supports multi-repo work items.
