...
pub fn draw_pane_output(buf: &mut Buffer, app: &App, theme: &Theme, area: Rect) {
    ...
    let title_line = build_output_pane_title(app, theme);
    ...
    if app.right_panel_tab == RightPanelTab::Terminal {
        draw_terminal_pane(buf, app, theme, block, area);
        return;
    }

    match selected_entry {
        Some(DisplayEntry::WorkItemEntry(wi_idx)) => {
            draw_work_item_entry(buf, app, theme, block, area, *wi_idx);
        }
        ...
        Some(DisplayEntry::ReviewRequestItem(rr_idx)) => {
            draw_review_request_entry(buf, app, theme, block, area, *rr_idx);
        }
        _ => {
...
/// Build the right-panel title line. Shows the active-session /
/// Terminal tab pair when the selected work item has a worktree, or
/// a single session-name line otherwise. Appends `[SCROLLBACK]` or
/// `[INPUT]` so the user can see the current focus / scrollback
/// state at a glance.
fn build_output_pane_title<'a>(app: &App, theme: &Theme) -> Line<'a> {
...
/// Render a selected `ReviewRequest` row in the right panel. Builds
/// the authoritative reviewer-identity list ("you" first when
/// directly requested, then other direct reviewers, then `team <slug>`
/// entries) and delegates to `draw_importable_pr_detail`.
fn draw_review_request_entry(
    buf: &mut Buffer,
    app: &App,
    theme: &Theme,
    block: Block<'_>,
    ...
    let Some(rr) = app.review_requested_prs.get(rr_idx) else {
        let text = Text::from(vec![
            Line::from(""),
...
/// Render the Terminal PTY in the right panel. Handles the live
/// session (locked parser + optional selection overlay), the ended
/// session hint, and the "Starting terminal..." placeholder when the
/// session has not been spawned yet.
fn draw_terminal_pane(buf: &mut Buffer, app: &App, theme: &Theme, block: Block<'_>, area: Rect) {
...
/// Render the right panel for a work-item row. Handles the rebase
/// gate spinner, review gate spinner, live / dead agent session PTY,
/// the worktree-creation spinner, and the fallback detail view /
/// error list.
fn draw_work_item_entry(
    buf: &mut Buffer,
    app: &App,
    theme: &Theme,
    block: Block<'_>,
    ...
    match session_entry {
        Some(entry) if !entry.alive => draw_session_ended(buf, theme, block, area),
        Some(entry) => draw_live_session(buf, theme, block, area, entry),
        None => draw_no_session(buf, app, theme, block, area, wi_idx),
    }
}

/// Render the "Session has ended" message shown in place of a live
/// PTY when the work item's agent session has exited.
fn draw_session_ended(buf: &mut Buffer, theme: &Theme, block: Block<'_>, area: Rect) {
    let text = Text::from(vec![
        Line::from(""),
...
/// Render a live work-item agent PTY: locks the shared parser, clamps
/// the scrollback offset to avoid vt100 underflow panics, renders the
/// `PseudoTerminal` widget, and paints any active text selection.
fn draw_live_session(
    buf: &mut Buffer,
    theme: &Theme,
    block: Block<'_>,
    area: Rect,
    entry: &crate::work_item::SessionEntry,
) {
...
/// Render the right panel for a work item with no live agent
/// session. Shows either the worktree-creation spinner (when a
/// background worktree create is in flight), a formatted error list
/// (when the work item has assembly errors), or the default detail
/// view with optional poll-error footer.
fn draw_no_session(
    buf: &mut Buffer,
    app: &App,
    theme: &Theme,
    ...
    if let Some(errors) = errors {
        draw_work_item_errors(buf, theme, block, area, wi, errors);
    } else {
...
/// Render a two-line gate / spinner panel: a title line with a
/// rotating spinner character and a secondary progress line below.
/// Used for the rebase gate, review gate, and worktree-creation
/// placeholders in the right panel.
fn draw_gate_spinner(
    buf: &mut Buffer,
    theme: &Theme,
    block: Block<'_>,
    area: Rect,
    ...
    let frame = spinner_tick % spinner_chars.len();
    let spinner = spinner_chars[frame] as char;
    let text = Text::from(vec![
        Line::from(""),
        Line::from(format!("  {spinner} {title}")),
        Line::from(""),
        Line::from(format!("  {progress}")),
    ]);
    let paragraph = Paragraph::new(text)
        .block(block)
        .style(theme.style_text_muted());
...
/// Render the "Errors" list for a work item with assembly failures.
/// Appends a stage-appropriate hint so the user knows how to proceed
/// once they have fixed the underlying problem.
fn draw_work_item_errors(
    buf: &mut Buffer,
    theme: &Theme,
    block: Block<'_>,
    area: Rect,
    wi: Option<&crate::work_item::WorkItem>,
    errors: &[crate::work_item::WorkItemError],
) {
    let mut lines = vec![
        Line::from(""),
    ...
    let hint_lines: &[&str] = match wi.map(|w| &w.status) {
        Some(WorkItemStatus::Backlog) => &["  Press c (Claude) or x (Codex) to begin planning."],
