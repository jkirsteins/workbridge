...
pub fn draw_work_item_list(buf: &mut Buffer, app: &App, theme: &Theme, area: Rect) {
    ...
    if app.display_list.is_empty() {
        draw_empty_work_list(buf, theme, block, area, app.board_drill_stage.is_some());
        return;
    }
    ...
    let items = build_work_list_items(app, theme, inner_width);
    ...
    let (body_area, sticky_slot, resolved_offset, max_item_offset) =
        resolve_viewport_layout(app, inner, &item_heights, selected, drill_down);
    ...
    // without redoing the layout math.
    register_row_click_targets(app, body_area, &item_heights, resolved_offset);
    ...
    if !drill_down {
        draw_sticky_group_header(buf, app, theme, sticky_slot, resolved_offset);
    }

    draw_work_list_scrollbar(
        buf,
        theme,
        ScrollbarArgs {
            area,
    ...
    draw_offscreen_selection_marker(
        buf,
        theme,
        OffscreenMarkerArgs {
            area,
...
/// Build the left-panel title string. Shows `" Work Items (N) "` in
/// the normal view and `" <Stage> (N) "` when drilled down into a
/// single board stage.
fn work_list_title(app: &App) -> String {
    app.board_drill_stage.as_ref().map_or_else(
...
/// Render the empty-state placeholder shown when the display list
/// has no entries. Uses a different hint set for the drill-down view
/// vs. the root view.
fn draw_empty_work_list(
    buf: &mut Buffer,
    theme: &Theme,
    block: Block<'_>,
    area: Rect,
...
/// Resolve the tentative-then-final viewport layout for one render
/// pass: runs the recenter pass against `inner.height` (so the
/// sticky-header decision can be made against the tentative offset),
/// decides whether to reserve a sticky-header slot, and then
/// reconciles the offset with the final body height. Returns
/// `(body_area, sticky_slot, resolved_offset, max_item_offset)`.
fn resolve_viewport_layout(
    app: &App,
    inner: Rect,
    item_heights: &[usize],
    selected: Option<usize>,
    drill_down: bool,
) -> (Rect, Option<Rect>, usize, usize) {
...
/// Inputs for `draw_work_list_scrollbar`. Bundled to stay under the
/// clippy `too_many_arguments` threshold.
#[derive(Clone, Copy)]
struct ScrollbarArgs<'a> {
    area: Rect,
    body_area: Rect,
    total_rows: usize,
    body_height: usize,
    resolved_offset: usize,
    item_heights: &'a [usize],
}

/// Render the work-item list scrollbar. Only painted when content
/// overflows the body or the list has been scrolled past the top.
/// Uses `body_area.height` (not `inner.height`) so the scrollbar
/// track matches whichever area the `List` was rendered into.
fn draw_work_list_scrollbar(buf: &mut Buffer, theme: &Theme, args: ScrollbarArgs<'_>) {
    let ScrollbarArgs {
        area,
        body_area,
        total_rows,
...
/// Inputs for `draw_offscreen_selection_marker`. Bundled to keep the
/// helper under clippy's `too_many_arguments` threshold.
#[derive(Clone, Copy)]
struct OffscreenMarkerArgs<'a> {
    area: Rect,
    body_area: Rect,
    body_height: usize,
    resolved_offset: usize,
...
/// Offscreen-selection marker: when the selected item is outside the
/// visible viewport, paint a single distinct-coloured cell in the
/// scrollbar column at the y-coordinate that corresponds to the
/// selection's position within the full list. This gives the user a
/// visual cue of where their keyboard selection sits relative to the
/// mouse-scrolled viewport. When the selection is visible, no marker
/// is painted - the normal thumb is enough.
fn draw_offscreen_selection_marker(buf: &mut Buffer, theme: &Theme, args: OffscreenMarkerArgs<'_>) {
    let OffscreenMarkerArgs {
        area,
        body_area,
        body_height,
        resolved_offset,
    ...
    if idx >= item_heights.len() || body_area.width == 0 || body_area.height == 0 {
        return;
    }
    ...
    // `max_row_offset == 0` means the whole list fits (no scrolling
    // possible) - guarded above but keep the divisor non-zero.
    let denom = max_row_offset.max(1);
    let marker_row = (row_of_selection.saturating_mul(body_area.height as usize - 1)) / denom;
...
/// Paint the reserved sticky-header slot (if any) using the
/// authoritative offset written to `list_scroll_offset`. Because the
...
/// reconciliation in the caller, the sticky decision and the
/// post-render offset should agree. The only legitimate drift is a
/// one-item sticky decision flip caused by the recenter shrinking the
/// body by 1 row, in which case `header_needed` still matches
/// `sticky_slot`. Treat any remaining mismatch as a one-frame visual
/// glitch rather than a hard assertion - the next frame will
/// reconcile. `(None, None)`: no header needed.
fn draw_sticky_group_header(
    buf: &mut Buffer,
    app: &App,
    theme: &Theme,
    sticky_slot: Option<Rect>,
    resolved_offset: usize,
) {
...
/// Build the `ListItem` vec for the work-item list. When a row is
/// the selected row, the row's background is painted with
/// `style_tab_highlight_bg` directly on the `ListItem` so the `List`
/// widget itself no longer owns the selection highlight.
/// `ListState::select` is deliberately NOT called by the caller -
/// decoupling the viewport from the selection means the renderer must
/// not let ratatui's `get_items_bounds` force an auto-scroll. The
/// highlight is a styling concern, the viewport a state concern, and
/// the two must stay independent for the wheel scroll to work without
/// snapping back.
fn build_work_list_items<'a>(app: &'a App, theme: &Theme, inner_width: usize) -> Vec<ListItem<'a>> {
    app.display_list
        .iter()
        .enumerate()
        .map(|(i, entry)| {
...
/// Push one `ClickTarget::WorkItemRow` per selectable visible row.
/// Only selectable entries (not group headers) are registered, so a
/// click on a group header falls through to the list-scroll / geometric
/// routing in `handle_mouse` instead of stealing the dispatch.
/// Offscreen rows are skipped - the registry hit-test is a linear
/// scan, so keeping it small keeps the mouse path cheap.
fn register_row_click_targets(
    app: &App,
    body_area: Rect,
    item_heights: &[usize],
    resolved_offset: usize,
