...
/// Pane rows = total rows minus 2 for borders, minus 1 if the status bar is visible.
pub fn compute(cols: u16, rows: u16, has_status_bar: bool) -> PanelLayout {
    ...
    // Subtract 1 extra row when the status bar is visible, since it steals
    // a row from the main area.
    let status_offset: u16 = if has_status_bar { 1 } else { 0 };
    let row_raw = rows.saturating_sub(2).saturating_sub(status_offset);
...
mod tests {
    ...
    #[test]
    fn standard_terminal_with_status_bar() {
        let pl = compute(80, 24, true);
        assert_eq!(pl.left_width, 20);
        assert_eq!(pl.pane_cols, 58);
        // rows = 24 - 2 (borders) - 1 (status) = 21
        assert_eq!(pl.pane_rows, 21);
    }

    #[test]
    fn wide_terminal_200x50() {
        let pl = compute(200, 50, false);
    ...
    fn zero_dimensions_produce_minimum_1() {
        let pl = compute(0, 0, false);
        assert_eq!(pl.pane_cols, 1);
        assert_eq!(pl.pane_rows, 1);
    }

    #[test]
    fn status_bar_on_tiny_terminal() {
        let pl = compute(80, 2, true);
        // rows = 2 - 2 (borders) - 1 (status) = 0 saturated -> 1
        assert_eq!(pl.pane_rows, 1);
    }
}
