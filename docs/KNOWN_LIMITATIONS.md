...
## Paste in legacy terminals without bracketed-paste support

**What:** workbridge's modal text inputs receive paste events via
`crossterm::event::Event::Paste`, which is only generated by terminals
that support bracketed paste (CSI `?2004h` / `?2004l`). Modern
...
burst instead of a single `Event::Paste`.

**Why it is accepted:** Character content still inserts via the
per-key `insert_char` path, so short text pastes do work in reduced
form. The broken case is multi-line paste into a single-line input:
a pasted newline arrives as `Event::Key(Enter)` and submits the
dialog (Create, Set-branch, Rework reason, Cleanup reason) or inserts
a real newline into the Description TextArea before the rest of the
payload arrives. Coalescing the burst into a synthetic paste event
would require timing heuristics that risk mis-batching real
fast-typing into false pastes, so we do not implement it.

**Impact:** Multi-line paste into single-line dialog inputs triggers
early submission on the first newline and the remaining characters
end up on the next screen (or trigger a second modal). Single-line
paste works correctly in every terminal.

**Workaround:** Use a modern terminal emulator with bracketed paste
enabled (macOS Terminal.app / iTerm2 / WezTerm / Ghostty, Windows
Terminal, Linux GNOME Terminal / Konsole / Alacritty / kitty). For
ssh setups, confirm the remote shell does not strip `?2004h`; most
recent `bash`/`zsh` do not.

**Future fix options:**
- Detect bursts of `Event::Key` with zero inter-event delay and
  coalesce them into a synthetic paste once the burst settles.
- Probe the terminal's bracketed-paste support on startup and surface
  a warning banner when it is absent.
